...
function notify_all(student_ids: array, message: string, title: string):
    ...
    begin_transaction()
        ...
        # bulk insert recipient rows (status tracked per channel)
        ...
        # transactional outbox event (guarantees enqueue after commit)
        insert_outbox_event(
...
# Dispatcher workers (horizontal scalable consumers)
...
# Stage 6 - Reusable Logging Middleware (Test Server Integration)

## Objective
Build a reusable logging component that sends structured logs to the protected test-server log API:
- registration endpoint to get `clientID` and `clientSecret`
- auth endpoint to get bearer token
- log endpoint to submit logs from backend application flow

## APIs Provided by Test Server

### 1) Registration API (one-time)
...
- `githubUsername`
- `rollNo`
- `accessCode` (must be your real code shared by email)

Output contains `clientID` and `clientSecret` (store securely; cannot be retrieved again).

### 2) Authorization Token API
...
Components:
1. **CredentialStore**
   - stores registration/auth inputs and secrets in environment variables.
2. **TokenManager**
   - fetches token on startup or first log call.
   - refreshes token before expiry.
3. **LogValidator**
   - validates `stack`, `level`, `package`, and non-empty `message`.
4. **LogClient**
   - calls `/evaluation-service/logs` with bearer token.
5. **Log function**
   - reusable API used everywhere: `Log(stack, level, package, message)`.
...
function get_access_token():
    ...
    # refresh 60s early to avoid race near expiry
    if token_cache.access_token != null and now < token_cache.expires_at_epoch - 60:
        return token_cache.access_token

    auth_body = {
        "email": ENV.LOG_EMAIL,
        "name": ENV.LOG_NAME,
        "rollNo": ENV.LOG_ROLL_NO,
        "accessCode": ENV.LOG_ACCESS_CODE,
        "clientID": ENV.LOG_CLIENT_ID,
        "clientSecret": ENV.LOG_CLIENT_SECRET
    }

    res = http_post("http://20.207.122.201/evaluation-service/auth", auth_body)
    token_cache.access_token = res.access_token
    token_cache.expires_at_epoch = now + res.expires_in
    return token_cache.access_token


function validate_log_input(stack, level, package_name, message):
    if stack not in ALLOWED_STACK:
        raise ValidationError("invalid stack")
    if level not in ALLOWED_LEVEL:
        raise ValidationError("invalid level")
    if package_name not in ALLOWED_PACKAGE:
        raise ValidationError("invalid package")
    if message == null or trim(message) == "":
        raise ValidationError("message required")


function Log(stack, level, package_name, message):
    try:
        validate_log_input(stack, level, package_name, message)
    except ValidationError as err:
        # never crash business flow due to logging misuse
        console_error("log validation failed: " + err.message)
        return
    ...
    try:
        ...
        # token may be expired/revoked; force one refresh and retry once
        ...
        # fail-open: app logic should continue even if log server is down
        console_error("log push failed: " + err.message)
```

## Where to Call `Log(...)` in Backend

Suggested usage:
- Route entry: request received with key identifiers.
- Validation failure: include field + reason.
- Controller start/end: operation begin + success summary.
- DB errors: query/action context and error class.
- External API failure: endpoint + status + timeout.
- Catch blocks: structured fatal/error logs with context.

Examples:
```python
Log("backend", "info", "route", "POST /notifications/notify-all received")
Log("backend", "warn", "handler", "invalid student_id format in request body")
Log("backend", "error", "db", "insert into student_notifications failed")
Log("backend", "fatal", "service", "notify_all worker crashed due to OOM")
```

## Reliability Notes

- Keep logger **non-blocking/fail-open**: app must not fail because log API fails.
- Add small timeout and at most one retry for log push.
- Redact secrets/PII from messages before sending.
- Use structured, specific messages (operation + entity + reason), not generic text.

## Stage 6 Outcome

Now the system has a reusable, validated, token-aware logging middleware design aligned with the evaluation test-server contract and constraints.

# Stage 7 - Priority Inbox (Python Implementation)
...
The script supports:
- direct token with `EVAL_ACCESS_TOKEN`
- or token fetch through auth API using env credentials (`EVAL_EMAIL`, `EVAL_NAME`, `EVAL_ROLL_NO`, `EVAL_ACCESS_CODE`, `EVAL_CLIENT_ID`, `EVAL_CLIENT_SECRET`)
